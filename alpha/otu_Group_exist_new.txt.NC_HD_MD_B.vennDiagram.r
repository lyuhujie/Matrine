...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_B.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_B.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_B.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_B.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("NCB" != "CHENTONG"){
        NCB <- data[grepl("\\<NCB\\>",data[,2]),1]
        num <- num + 1
    }

    if("HDB" != "CHENTONG"){
        HDB <- data[grepl("\\<HDB\\>",data[,2]),1]
        num <- num + 1
    }

    if("MDB" != "CHENTONG"){
        MDB <- data[grepl("\\<MDB\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(NCB=NCB, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, HDB=HDB,
                     MDB=MDB),
        ...
        p <- venn.diagram(
            x = list(NCB=NCB, CHENTONG=CHENTONG, HDB=HDB,
                     MDB=MDB),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(NCB=NCB, HDB=HDB, MDB=MDB),
