...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_A.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_A.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_A.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_A.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("NCA" != "CHENTONG"){
        NCA <- data[grepl("\\<NCA\\>",data[,2]),1]
        num <- num + 1
    }

    if("HDA" != "CHENTONG"){
        HDA <- data[grepl("\\<HDA\\>",data[,2]),1]
        num <- num + 1
    }

    if("MDA" != "CHENTONG"){
        MDA <- data[grepl("\\<MDA\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(NCA=NCA, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, HDA=HDA,
                     MDA=MDA),
        ...
        p <- venn.diagram(
            x = list(NCA=NCA, CHENTONG=CHENTONG, HDA=HDA,
                     MDA=MDA),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(NCA=NCA, HDA=HDA, MDA=MDA),
