...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_C.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_C.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_C.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_Group_exist_new.txt.NC_HD_MD_C.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if("NCC" != "CHENTONG"){
        NCC <- data[grepl("\\<NCC\\>",data[,2]),1]
        num <- num + 1
    }

    if("HDC" != "CHENTONG"){
        HDC <- data[grepl("\\<HDC\\>",data[,2]),1]
        num <- num + 1
    }

    if("MDC" != "CHENTONG"){
        MDC <- data[grepl("\\<MDC\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(NCC=NCC, CHENTONG=CHENTONG,
                     CHENTONG=CHENTONG, HDC=HDC,
                     MDC=MDC),
        ...
        p <- venn.diagram(
            x = list(NCC=NCC, CHENTONG=CHENTONG, HDC=HDC,
                     MDC=MDC),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(NCC=NCC, HDC=HDC, MDC=MDC),
