...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_Group_exist_new.txt.HD_ABC.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_Group_exist_new.txt.HD_ABC.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_Group_exist_new.txt.HD_ABC.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_Group_exist_new.txt.HD_ABC.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(HDA=HDA, CHENTONG=CHENTONG,
        ...
        p <- venn.diagram(
            x = list(HDA=HDA, CHENTONG=CHENTONG, HDB=HDB,
                     HDC=HDC),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(HDA=HDA, HDB=HDB, HDC=HDC),
