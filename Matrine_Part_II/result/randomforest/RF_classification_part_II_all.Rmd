...
sub_metadata2 = subset(metadata_all, group %in% c(State_A,State_B))
sub_metadata2
...
# 设置训练集
train = subset(metadata_f, Group_2 %in% c("LMATA", "HMATA"))
train

train2 = train
train2
train2$group = as.factor(train2$group)
summary(train2)
...
for(i in c("2Phylum","3Class","4Order","5Family","6Genus","8OTU0.02")){
  # i="5Family"
  set.seed(0)
  table_f = read.table(paste0("tax_LMAT_HMAT_",i,".txt"),header = T, row.names = 1)
  table_f = table_f[,rownames(train2)]
  train2$group = as.factor(train2$group)
  rf = randomForest(t(table_f), train2$group, importance=T, proximity=T, ntree = 1000)
  print(i)
  print(rf)
}
...
table =read.table("tax_LMAT_HMAT_5Family.txt",header = T, row.names = 1)
# train2重命名，方便后面区分。
...
for (i in 0:9){
  set.seed(i)
  rf2 = randomForest(t(otu_sub1), metadata_train1$group, importance=TRUE, proximity=TRUE, ntree = 1000)
  print(i)
  print(rf2)
}
...
library(randomForest)
set.seed(5)
rf3 = randomForest(t(otu_sub1), metadata_train1$group, importance=TRUE, proximity=TRUE, ntree = 1000)
print(rf3)
```
...
result = rfcv(t(otu_sub1), metadata_train1$group, cv.fold=5)
...
for (i in 1:(1+9)){
  print(i)
  set.seed(i)
  result= rfcv(t(otu_sub1), metadata_train1$group, cv.fold=5) #  scale = "log", step = 0.9
...
```{r}
test = subset(metadata_f, Group_2 %in% c("HMATB", "LMATB")) 
...
otutab_t2$group = metadata_test[rownames(otutab_t2),]$group
```
...
pre_tab = table(observed=otutab_t2[,"group"],
...
```{r}
# 整理样本原始分组和预测分类
predict = data.frame(group = otutab_t2[,"group"], predicted=otutab.pred)
...
# IND=1, TEJ=2
predict$predict = ifelse(predict$predicted == "CLMAT", 1, 2)
# Set sample number in each row
column1 = 3
column = 3

AA1 = predict[predict$group=="CLMAT",]$predict
length(AA1)
...
BB1 = predict[predict$group=="DHMAT",]$predict
length(BB1)
