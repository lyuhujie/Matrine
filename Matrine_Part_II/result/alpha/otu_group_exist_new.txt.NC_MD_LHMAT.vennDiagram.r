...
if ("pdf" == "png") {
    png(filename="result/alpha/otu_group_exist_new.txt.NC_MD_LHMAT.vennDiagram.png", width=3, height=3,
        res=300, units="cm")
} else if ("pdf" == "eps") {
    postscript(file="result/alpha/otu_group_exist_new.txt.NC_MD_LHMAT.vennDiagram.eps", onefile=FALSE, horizontal=FALSE,
               paper="special", width=10, height = 12, pointsize=10)
} else if ("pdf" == "pdf") {
    pdf(file="result/alpha/otu_group_exist_new.txt.NC_MD_LHMAT.vennDiagram.pdf", width=3, height=3)
}else if ("pdf" == "svg") {
    svg(filename="result/alpha/otu_group_exist_new.txt.NC_MD_LHMAT.vennDiagram.svg", width=3, height=3,
        pointsize=10)
} else {
...
if (! FALSE) {
    ...
    data <- read.table(file="result/alpha/otu_group_exist_new.txt", sep="\t", quote="")

    num <- 0

    if("ANC" != "CHENTONG"){
        ANC <- data[grepl("\\<ANC\\>",data[,2]),1]
        num <- num + 1
    }

    if("BMD" != "CHENTONG"){
        BMD <- data[grepl("\\<BMD\\>",data[,2]),1]
        num <- num + 1
    }

    if("CLMAT" != "CHENTONG"){
        CLMAT <- data[grepl("\\<CLMAT\\>",data[,2]),1]
        num <- num + 1
    }

    if("DHMAT" != "CHENTONG"){
        DHMAT <- data[grepl("\\<DHMAT\\>",data[,2]),1]
        num <- num + 1
    }
    ...
    if(num == 5){
        p <- venn.diagram(
            x = list(ANC=ANC, DHMAT=DHMAT,
                     CHENTONG=CHENTONG, BMD=BMD,
                     CLMAT=CLMAT),
        ...
        p <- venn.diagram(
            x = list(ANC=ANC, DHMAT=DHMAT, BMD=BMD,
                     CLMAT=CLMAT),
            filename = NULL, col = "black", lwd = 1,
            fill = color_v,
            alpha = 0.50, main="",
        ...
        p <- venn.diagram(
            x = list(ANC=ANC, BMD=BMD, CLMAT=CLMAT),
