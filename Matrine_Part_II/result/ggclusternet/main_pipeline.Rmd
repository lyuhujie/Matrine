...
dir.create(path_1)
# 作为一个data.frame，出现，。
write.table(otu_table,file = "./imitate/otu_table.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
tax_table = ps_net %>%
  vegan_tax() %>%
  as.data.frame()
write.table(tax_table,file = "./imitate/tax_table.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggsave("./imitate/8group_cs1.pdf",po1,width = 8,height = 6)
```
...
ggsave("./imitate/1group_cs2.pdf",pnet1,width = 7,height = 5.5)
...
tax
write.table(tax,file = "./species_class/tax_with_filed.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggsave("./species_class/raw_group_Phylum_cs1.pdf",pnet3,width = 7,height = 5.5)
...
ggsave("./species_class/optimize1_group_cs2.pdf",pnet4,width = 7,height = 5.5)
...
ggsave("./species_class/optimize2_group_cs3.pdf",pnet5,width = 7,height = 5.5)
```
...
ggsave("./species_class/optimize3_maptree_group_cs4.pdf",pnet6,width = 7,height = 5.5)
```
...
write.table(nodes2,file = "./net_model/tax_with_model.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggsave("./net_model/model_maptree_group_cs1.pdf",pnet7,width = 7,height = 5.5)
...
ggsave("./net_model/model_maptree2_group_cs2.pdf",pnet8,width = 7,height = 5.5)
...
write.table(dat,file = "./igraphi/tax_with_modelname.txt",quote = F,sep = '\t', row.names = T, col.names = T)
write.table(tem,file = "./igraphi/tax_with_color.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
nodes = nodeadd(plotcord =node,otu_table = otu_table,tax_table = tax_table)
head(nodes)
write.table(nodes,file = "./igraphi/nodes_with_class.txt",quote = F,sep = '\t', row.names = T, col.names = T)
#-----计算边#--------
edge = edgeBuild(cor = cor,node = node)
colnames(edge)[8] = "cor"
head(edge)
...
ggsave("./igraphi/igraphi_cs1.pdf",plo1,width = 12,height = 10)
...
head(dat1)
write.table(dat1,file = "./net_node_features/net_feature1.txt",quote = F,sep = '\t', row.names = T, col.names = T)


# 升级后包含的网络属性更多, n.hub中T不行跑不出来，F可以
dat2 = net_properties.2(igraph,n.hub = F)
head(dat2,n = 16)
write.table(dat2,file = "./net_node_features/net_feature2.txt",quote = F,sep = '\t', row.names = T, col.names = T)

dat3 = net_properties.3(igraph,n.hub = F)
head(dat3,n = 16)
write.table(dat3,file = "./net_node_features/net_feature3.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
write.table(dat4,file = "./net_node_features/net_feature4.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
head(nodepro)
write.table(nodepro,file = "./net_node_features/nodes_feature.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
pA
dataNN <- res[[2]]
write.table(dataNN,file = "./net_node_features/zipi_data.txt",quote = F,sep = '\t', row.names = T, col.names = T)
ggsave("./net_node_features/zipi_classfication.pdf",pA,width = 8,height = 7)
...
ggsave("./net_node_features/top_keystone_micro.pdf",width = 5,height = 4)
...
path = "./result_1000_igraph2_4_500/"
dir.create(path)
