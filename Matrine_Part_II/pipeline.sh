...
### 6.3. 筛选高丰度菌 Filter by abundance
    ...
    # 按照自己分组筛选高丰度菌。按照Group_all列分组
    Rscript ${db}/script/otu_mean.R --input result/otutab.txt \
      --metadata result/metadata.txt \
      --group Group_all --thre 0 \
      --scale TRUE --zoom 100 --all FALSE --type mean \
      --output result/otutab_mean_all.txt
    # 结果为全部和各组均值
    head -n3 result/otutab_mean_all.txt
    ...
    # awk 'BEGIN{OFS=FS="\t"}{if(FNR==1) {for(i=2;i<=NF;i++) a[i]=$i; print "OTU","Group";} \
    #     else {for(i=2;i<=NF;i++) if($i>0.1) print $1, a[i];}}' \
    #     result/otutab_mean_all.txt > result/alpha/otu_all_exist_new.txt
    # head result/alpha/otu_all_exist_new.txt
    # cut -f 2 result/alpha/otu_all_exist_new.txt | sort | uniq -c
...
## 7. β多样性 Beta diversity
    ...
    usearch -beta_div result/otutab_mean_new.txt -tree result/otus.tree \
      -filename_prefix result/beta2/

    mkdir -p result/beta3/
    usearch -cluster_agg result/otus.fa -treeout result/otus.tree
    #生成5种距离矩阵：bray_curtis, euclidean, jaccard, manhatten, unifrac
    usearch -beta_div result/otutab_mean_all.txt -tree result/otus.tree \
      -filename_prefix result/beta3/
...
### 1.1 Alpha多样性箱线图
    ...
    Rscript ${db}/script/alpha_boxplot.R --alpha_index richness \
      ...
      --group group --output result/alpha/new_ \
      --width 89 --height 59
    # 使用循环绘制6种常用指数
...
### 1.3 多样性维恩图
    ...
    bash ${db}/script/sp_vennDiagram.sh \
      -f result/alpha/otu_group_exist_new.txt \
      -a ANC -b BMD -c CLMAT -d DHMAT \
      -w 3 -u 3 \
      -p NC_MD_LHMAT
...
### 2.1 距离矩阵热图pheatmap
    ...
    bash ${db}/script/sp_pheatmap.sh \
      -f result/beta3/bray_curtis.txt \
      -H 'TRUE' -u 6 -v 5
    # 添加分组注释，如2，4列的基因型和地点
    ...
    # 见脚本 C:\Matrine_part_I\result\beta2\Figure_heatmap_all.Rmd(Figure_heatmap_group.Rmd)
...
### 2.2 主坐标分析PCoA
    ...
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta3/bray_curtis.txt --design result/metadata.txt \
      --group Group --label FALSE --width 89 --height 59 \
      --output result/beta2/bray_curtis.pcoa.pdf
    # 添加样本标签 --label TRUE
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta3/bray_curtis.txt --design result/metadata.txt \
    ...
    ## jaccard方式
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta3/jaccard.txt --design result/metadata.txt \
      --group Group --label FALSE --width 89 --height 59 \
      --output result/beta2/jaccard.pcoa.pdf
    # 添加样本标签 --label TRUE
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta3/jaccard.txt --design result/metadata.txt \
      --group Group --label TRUE --width 89 --height 59 \
      --output result/beta2/jaccard.pcoa.label.pdf
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot.R \
      ...
      --group group --color manual1 --legend 12 --width 181 --height 119 \
      --output result/tax/sum_p_group2_newcolor
    # 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    # 常规配色
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_p.txt --design result/metadata.txt \
      --group group --color Paired --legend 12 --width 181 --height 119 \
      --output result/tax/sum_p_group
    
    # 以属(g)水平为例，结果包括output.sample/group.pdf两个文件
    # 新颜色
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata.txt \
      --group group --color manual1 --legend 12 --width 181 --height 119 \
      --output result/tax/sum_g_group2_newcolor
    # 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    # 常规配色
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata.txt \
      --group group --color Paired --legend 12 --width 181 --height 119 \
      --output result/tax/sum_g_group
...
###  差异比较
    ...
    # 选择方法 wilcox/t.test/edgeR、pvalue(0.05)和fdr(0.2)和输出目录
    # 改threshold 有用0.15
    # 比较
    ...
    Rscript ${db}/script/compare.R \
      ...
      --group group --compare ${compare} --threshold 0.15\
    ...
    # 图例显示不图，可增加高度v为119+即可，后期用AI拼图为NN-NN.heatmap.emf
    # 可切换为纲c和-L Class展示细节
    # 门水平
    bash ${db}/script/compare_manhattan.sh -i result/compare/${compare}.txt \
       -t result/taxonomy.txt \
       -p result/tax/sum_p.txt \
       -w 190 -v 100 -s 7 -l 20 -L Phylum \
       -o result/compare/${compare}.manhattan.p2.pdf
    # 属水平。显示完整图例，再用AI拼图
    bash ${db}/script/compare_manhattan.sh -i result/compare/${compare}.txt \
       -t result/taxonomy.txt \
       -p result/tax/sum_g.txt \
       -w 190 -v 130 -s 7 -l 15 -L Genus \
       -o result/compare/${compare}.manhattan.g2.pdf


  ### 单个特征的绘制
    # 筛选显示差异ASV，按某一个组丰度降序列，取ID展示前10
    awk '$4<0.05' result/compare/ANC-BMD.txt | sort -k7,7nr | cut -f1 | head
    awk '$4<0.05' result/compare/ANC-CLMAT.txt | sort -k7,7nr | cut -f1 | head
    awk '$4<0.05' result/compare/ANC-DHMAT.txt | sort -k7,7nr | cut -f1 | head
    awk '$4<0.05' result/compare/CLMAT-DHMAT.txt | sort -k7,7nr | cut -f1 | head
    # 关键ASV在NC-MD组中。有 ASV_6 ASV_8 ASV_14 ASV_30 ASV_34 ASV_37 ASV_39 ASV_57 ASV_36 ASV_56 
    # 关键ASV在NC-LMAT组中。有 ASV_6 ASV_8 ASV_14 ASV_30 ASV_34 ASV_37 ASV_39 ASV_57 ASV_36 ASV_45 
    # 关键ASV在HD-HMAT组中。有 ASV_6 ASV_8 ASV_14 ASV_30 ASV_34 ASV_37 ASV_39 ASV_57 ASV_36 ASV_45 
    # 关键ASV在LMAT-HMAT组中。有 ASV_4 ASV_13 ASV_41 ASV_54 ASV_71 ASV_19 ASV_43 ASV_85 ASV_48 ASV_92 
    
  ### HD-MD中的高丰度ASV。
    # 差异OTU细节展示。按照大的分组方式
    # 按照细分的分组方式
    for i in ASV_6 ASV_8 ASV_14 ASV_30 ASV_34 ASV_37 ASV_39 ASV_57 ASV_36 ASV_56; do
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ${i} \
      --input result/otutab.txt --design result/metadata.txt \
      --transpose TRUE --scale TRUE \
      --width 89 --height 59 \
      --group group --output result/compare/feature_group_NCMD; done
    
    
    # HD-MD, HD-LMAT, HD-HMAT可能有相同的微生物，可以一起出不用一个一个出。数据量会太大。
    for i in ASV_6 ASV_8 ASV_14 ASV_30 ASV_34 ASV_37 ASV_39 ASV_57 ASV_36 ASV_56 ASV_45; do
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ${i} \
      --input result/otutab.txt --design result/metadata.txt \
      --transpose TRUE --scale TRUE \
      --width 89 --height 59 \
      --group group --output result/compare/feature_group_all; done
    
    # 在LMAT-HMAT中，高丰度的微生物箱线图。
    for i in ASV_4 ASV_13 ASV_41 ASV_54 ASV_71 ASV_19 ASV_43 ASV_85 ASV_48 ASV_92; do
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ${i} \
      --input result/otutab.txt --design result/metadata.txt \
      --transpose TRUE --scale TRUE \
      --width 89 --height 59 \
      --group group --output result/compare/feature_group_LHMAT; done  
    
    
  ### 物种分类水平，按照物种名称来画
    # 指定某列排序：按属丰度均值All降序
    csvtk -t sort -k All:nr result/tax/sum_g.txt | head -15
    # 筛选出来的前15的微生物
    # Kineothrix Lacrimispora Flintibacter Roseburia Blautia Enterocloster Acetatifactor Oscillibacter Clostridium_XlVa Duncaniella Bacteroides Paramuribaculum Clostridium_IV
    
    # 差属细节展示。按照较细的分组
    for i in Kineothrix Lacrimispora Flintibacter Roseburia Blautia Enterocloster Acetatifactor Oscillibacter Clostridium_XlVa Duncaniella; do
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ${i} \
      --input result/tax/sum_g.txt --design result/metadata.txt \
      --transpose TRUE \
      --width 89 --height 59 \
      --group group --output result/compare/feature_group; done
...
### 2.2 绘制扩展柱状图和表
  ...
  ### 组间比较。NC和MD组比较
    ...
    Rscript ${db}/script/compare_stamp.R \
      ...
      --group group --compare ${compare} --threshold 0.6 \
      --method "t.test" --pvalue 0.05 --fdr "none" \
      --width 189 --height 80 \
      --output result/stamp/${compare}
...
# 31、功能预测
  ### PICRUSt 2.0
    ...
    # 进入工作目录，服务器要修改工作目录
    wd=/mnt/c/Matrine_part_II/result/picrust2
    #wd=/mnt/c/Matrine_part_I/result/picrust2/out
    mkdir -p ${wd} && cd ${wd}
...
## 2. 元素循环FAPROTAX
    ...
    # 设置工作目录
    wd=/mnt/c/Matrine_part_II/result/faprotax2/
    mkdir -p ${wd} && cd ${wd}
    # 设置脚本目录
...
## 3. Bugbase细菌表型预测
    ...
    Rscript ${bugbase}/bin/run.bugbase.r -L ${bugbase} \
      -i gg/otutab.txt -m metadata.txt -c group -o bugbase2/
...
## 1. 筛选高丰度/指定的特征
    ...
    awk 'NR==FNR{a[$1]=$0} NR>FNR{print a[$1]}' ../otutab_mean_new.txt otutab_high.id \
